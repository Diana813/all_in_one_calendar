...
import com.dianaszczepankowska.AllInOneCalendar.android.events.expandedDayView.DoneOnEditorActionListener;
import com.dianaszczepankowska.AllInOneCalendar.android.utils.CopyPasteEditText;
...
public class ExpandableListHoursAdapter extends BaseExpandableListAdapter {
    ...
    public ExpandableListHoursAdapter(Context context, List<String> expandableListTitle,
    ...
    public View getChildView(int listPosition, final int expandedListPosition,
        ...
        editText.addListener(() -> UtilsEvents.saveDataEvents(editText, currentDate, editText.getText().toString(), "-1", schedule, EventKind.EVENTS.getIntValue(), UtilsEvents.createEventId(currentDate)));
    ...
    private String findShiftSchedule() {
        ...
        if (calendarEventsDao.findBypickedDate(currentDate) != null) {
            String todaysShift = calendarEventsDao.findBypickedDate(currentDate).getShiftNumber();
    ...
    private String findPreviousShiftSchedule() {
        ...
        LocalDate yest = DateUtils.refactorStringIntoDate(currentDate).minusDays(1);
    ...
    private void editTextGroupListHandler(CopyPasteEditText editText, int listPosition, String listTitle) {
        ...
        editText.setOnLongClickListener(v -> {
            ...
            Event cyclicalEvent = eventsDao.findBySchedule(currentDate, listTitle);
            if (cyclicalEvent != null) {
                deleteCyclicalEvent.deleteCyclicalEventFromPickedDay(cyclicalEvent.getEvent_name(), eventsDao);
                CalendarProviderMethods.deleteEvent(context, cyclicalEvent.getEventId());
            }
            eventsDao.deleteBySchedule(currentDate, listTitle);
            editText.setText("");
            return false;
        });
    ...
    public void addDataFromDB(String listTitle, CopyPasteEditText editText) {

        EventsDao eventsDao = CalendarDatabase.getDatabase(context).eventsDao();
        Event event = eventsDao.findBySchedule(currentDate, listTitle);
        if (event != null) {
    ...
    private void handleEditTextSingleItem(CopyPasteEditText editText, int listPosition, int expandedListPosition, String expandedListText) {
        ...
        editText.setOnLongClickListener(v -> {
            EventsDao eventsDao = CalendarDatabase.getDatabase(context).eventsDao();
            Event cyclicalEvent = eventsDao.findBySchedule(currentDate, expandedListText);
            if (cyclicalEvent != null) {
                deleteCyclicalEvent.deleteCyclicalEventFromPickedDay(cyclicalEvent.getEvent_name(), eventsDao);
            }
            eventsDao.deleteBySchedule(currentDate, expandedListText);
            editText.setText("");
            return false;
        });
    }
