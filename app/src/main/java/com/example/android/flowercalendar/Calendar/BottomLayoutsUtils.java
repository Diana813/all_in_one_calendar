...
class BottomLayoutsUtils {
    ...
    void initShiftsData(CalendarFragment context, final BottomLayoutShiftsAdapter bottomLayoutShiftsAdapter) {
        ShiftsViewModel shiftsViewModel = ViewModelProviders.of(context).get(ShiftsViewModel.class);
        shiftsViewModel.getShiftsList().observe(context, new Observer<List<Shift>>() {
            @Override
            public void onChanged(@Nullable List<Shift> shifts) {
                bottomLayoutShiftsAdapter.setShiftList(shifts);
            ...
            public void onStateChanged(@NonNull View view, int newState) {
                switch (newState) {
                    case BottomSheetBehavior.STATE_HIDDEN:
                        break;
                    case BottomSheetBehavior.STATE_EXPANDED: {
                        BottomLayoutShiftsAdapter shiftsAdapter = new BottomLayoutShiftsAdapter(context, context);
